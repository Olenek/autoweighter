...
#-> Добавлено для совместимости с OData SAP
    @catch_requests_errors
...
# <-
    ...
    def execute_post(self, url, data, params=None):
        headers = {
# -> Изменено для совместимости с OData SAP
...
# -> Добавлено для совместимости с OData SAP
    def execute_put(self, url, data):
        ...
        self.log.info(u'PUT {0}'.format(url))
        self.log.info(u'Payload: {0}'.format(data))

        response = self._do_put(url, data=data, headers=headers)
        self._handle_odata_error(response)
# <-
